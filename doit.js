...
const scrape = async mySpecialUsername => {
	...
	while (count === 0 || dom.length > 0) {
		const temp = new JSDOM(
			await rp("http://slicice.net/search/albums.html?go=true&id=" + albumId + "&position=" + count),
		);
		...
		dom.forEach(el => {
			...
			const tempDuplicates = domBody
				.querySelector("p.duplicates")
				.textContent.replace(/ponujam: /g, "")
				.replace(/\s/g, "");
			let duplicates = tempDuplicates
				.split(",")
				.filter(el => {
					return el !== "";
				})
			...
			if (username === mySpecialUsername) {
				...
				console.log(
					`You are now missing ${padZeros(missing.length)} cards in your album and have ${padZeros(
						duplicates.length,
	...
	otherCollectors.forEach(collector => {
		...
		collector.duplicates.forEach(card => {
			if (myData.missing.includes(card)) {
				...
				if (!Array.isArray(myData.cardsByCollector[card])) {
					// initialize array if not already an array
					myData.cardsByCollector[card] = [];
				}
				if (!myData.cardsByCollector[card].includes(collector.username)) {
					// dont re-add again!
		...
		collector.recommendedTrade = calculateRecommended(
			collector.matchedMyMissing,
			collector.matchedMyDuplicates,
...
const autoTrade = (elementsOld, mySpecialDataOld) => {
	...
	while (mySpecialData.missing.length !== 0 && mySpecialData.duplicates.length !== 0) {
		...
		const useful = elements[elements.length - 1].recommendedTrade.map(trade => {
			// if there are no trades, .map wont run!
		...
		console.log(
			`You are now missing ${padZeros(mySpecialData.missing.length)} cards in your album and have ${padZeros(
				mySpecialData.duplicates.length,
			)} unique cards available for trade`,
		);
